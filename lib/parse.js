...
const join   = (ln, lineNum, lns) => {
  ...
  while(cont){
    ...
    if(newLn && (newLn[0] === ' ' 
      || (symbols.includes(newLn[0]) && isFn && !isFun(newLn)))) combined.push(newLn)
    else cont = false
  }
...
const JoinAndParseBody = body => {
  return body
  .map((ln, lineNum, lns) =>{
    return (isVar(ln)||isFun(ln)) && join(ln, lineNum,lns)
  })
  .filter(x => x)
  .map(ln => {
...
joinedAndParsed.filter(x => x.type === 'f' && x.name === 'solution').map(x => {
  ...
  x.body.where.filter(x => x.name === 'f').map (y => {
    // console.log(x.body)
    // console.log(x.body.where[3].body)
    // console.log(x.body.where[0].body)
    // console.log(x.body.where[0].body.where[1])


    // console.log('\n\n--------BREAK------------\n\n')
    // // console.log(y)
    // // console.log(y.body)
    // // console.log(y.body.where[0])
    // // console.log(y.body.where[0].body)
    console.log(y.body.where[0].body.where[1].body.where)
  })
})